...
public class Utils
{
	...
	/** Convenience overload: resolves entity type from an EntitySelector with command context. */
	...
	/**
	 * Resolves the entity type for NBT suggestions, with full context awareness.
	 * Tries in order: direct selector fields → /execute as context → local player fallback.
	 */
	...
	/**
	 * Scans the command input before {@code cursorPos} for {@code /execute as <target>} chains
	 * and resolves what entity type {@code @s} would refer to.
	 * Skips {@code @s} targets (inherits from outer scope) and handles nested {@code /execute}.
	 */
	...
	private static @Nullable String entityTypeFromRawTarget(String target)
	{
		...
		if (target.charAt(0) == '@' && target.length() >= 2)
		{
			...
			if (selType == 'e' && target.length() > 3 && target.charAt(2) == '[')
			{
				...
				if (typeIdx != -1)
				{
					...
					Identifier id = Identifier.tryParse(typeVal);
		...
		if (level != null)
		{
			...
			{
				if (player.getGameProfile().name().equals(target))
				{
					return "entity/" + EntityType.getKey(EntityType.PLAYER);
				}
			}
